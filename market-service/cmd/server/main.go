// Package main wires market-service's layers together and starts the server.
package main
...
import (
	...
	"p9e.in/samavaya/agriculture/market-service/api/v1/marketv1connect"
	grpcadapter "p9e.in/samavaya/agriculture/market-service/internal/adapters/inbound/grpc"
	kafkaadapter "p9e.in/samavaya/agriculture/market-service/internal/adapters/outbound/kafka"
	postgresadapter "p9e.in/samavaya/agriculture/market-service/internal/adapters/outbound/postgres"
	"p9e.in/samavaya/agriculture/market-service/internal/application"
)
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/market_service?sslmode=disable")
	...
	repo := postgresadapter.NewMarketRepository(pool, logger)
	pub := kafkaadapter.NewEventPublisher(kafkaProducer, logger)
	svc := application.NewMarketService(repo, pub, logger)
	handler := grpcadapter.NewMarketHandler(svc, logger)
	...
	path, marketHandler := marketv1connect.NewMarketServiceHandler(handler,
