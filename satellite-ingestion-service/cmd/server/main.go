// Package main wires all satellite-ingestion-service layers together and starts the HTTP server.
package main
...
import (
	...
	"p9e.in/samavaya/agriculture/satellite-ingestion-service/internal/handlers"
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/satellite_ingestion_service?sslmode=disable")
	...
	repo := repositories.NewIngestionRepository(d)
	svc := services.NewIngestionService(d, repo)
	handler := handlers.NewIngestionHandler(d, svc)
	...
	const serviceName = "satellite-ingestion-service"
	path, svcHandler := v1connect.NewSatelliteIngestionServiceHandler(handler,
	...
	go func() {
		p9log.NewHelper(logger).Infow("msg", "satellite-ingestion-service starting", "port", port)
	...
	<-quit
	p9log.NewHelper(logger).Infow("msg", "shutting down satellite-ingestion-service")
