...
func (r *yieldRepository) CreateYield(ctx context.Context, entity *domain.Yield) (*domain.Yield, error) {
	...
	row := r.queryRow(ctx,
		`INSERT INTO yield_predictions (uuid, tenant_id, farm_id, field_id, crop_id, season, year, status, is_active, created_by)
		VALUES ($1,$2,'','','',$3,0,$4,true,$5)
		RETURNING uuid, tenant_id, season, status, is_active, created_by, created_at, version`,
...
func (r *yieldRepository) GetYieldByUUID(ctx context.Context, uuid, tenantID string) (*domain.Yield, error) {
	row := r.queryRow(ctx,
		`SELECT uuid, tenant_id, season, status, is_active, created_by, created_at, version
		FROM yield_predictions WHERE uuid=$1 AND tenant_id=$2 AND deleted_at IS NULL`,
		uuid, tenantID,
	)
	e, err := scanYield(row)
	if err != nil {
...
func (r *yieldRepository) UpdateYield(ctx context.Context, entity *domain.Yield) (*domain.Yield, error) {
	row := r.queryRow(ctx,
		`UPDATE yield_predictions SET season=COALESCE(NULLIF($1,''),season), status=COALESCE(NULLIF($2,''),status),
...
func (r *yieldRepository) DeleteYield(ctx context.Context, uuid, tenantID, deletedBy string) error {
	return r.exec(ctx,
		`UPDATE yield_predictions SET deleted_at=NOW(), deleted_by=$1, is_active=false WHERE uuid=$2 AND tenant_id=$3`,
...
func (r *yieldRepository) CheckYieldExists(ctx context.Context, uuid, tenantID string) (bool, error) {
	var exists bool
	err := r.queryRow(ctx,
		`SELECT EXISTS(SELECT 1 FROM yield_predictions WHERE uuid=$1 AND tenant_id=$2 AND deleted_at IS NULL)`,
		uuid, tenantID,
	).Scan(&exists)
	return exists, err
}
...
func (r *yieldRepository) CheckYieldNameExists(ctx context.Context, name, tenantID string) (bool, error) {
	var exists bool
	err := r.queryRow(ctx,
		`SELECT EXISTS(SELECT 1 FROM yield_predictions WHERE season=$1 AND tenant_id=$2 AND deleted_at IS NULL)`,
		name, tenantID,
	).Scan(&exists)
	return exists, err
}
