...
- [ ] Surface the same explanations in the Flutter app — the codegen half of this is now done: the Dart SDK and `protoc-gen-dart` do install here, and the generated protos have been regenerated, so `Explanation` is available to the app. What remains is the Flutter UI that renders it, which needs the Flutter SDK rather than just Dart
