// Package main wires sustainability-service's layers together and starts the server.
package main
...
import (
	...
	"p9e.in/samavaya/agriculture/sustainability-service/api/v1/sustainabilityv1connect"
	grpcadapter "p9e.in/samavaya/agriculture/sustainability-service/internal/adapters/inbound/grpc"
	kafkaadapter "p9e.in/samavaya/agriculture/sustainability-service/internal/adapters/outbound/kafka"
	postgresadapter "p9e.in/samavaya/agriculture/sustainability-service/internal/adapters/outbound/postgres"
	"p9e.in/samavaya/agriculture/sustainability-service/internal/application"
)
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/sustainability_service?sslmode=disable")
	...
	if kafkaBroker != "" {
		...
		if err != nil {
			// A warning rather than a fatal: the accounting works without Kafka.
			// What is lost is the prohibited-input alert, which is how somebody
			// finds out about a broken organic conversion in the same week
			// rather than at the next audit — so the log says it plainly.
	...
	// One postgres type serves both repository ports: the input records and the
	// footprints computed from them are written in the same transactions and
	// read together, and splitting them would buy nothing but two pools.
	repo := postgresadapter.NewRepository(pool, logger)
	pub := kafkaadapter.NewEventPublisher(kafkaProducer, logger)

	svc := application.NewSustainabilityService(repo, repo, pub, logger)
	handler := grpcadapter.NewSustainabilityHandler(svc, logger)
	...
	path, sustainabilityHandler := sustainabilityv1connect.NewSustainabilityServiceHandler(handler,
	...
	mux.Handle(path, sustainabilityHandler)
