//! Image preprocessing for disease detection inference.
//!
//! Mirrors the Python validation/inference transforms from `disease_detection/transforms.py`:
//! 1. Resize to 282x282 (256 * 1.1)
...
/// Default input image size for disease detection (EfficientNet-B4).
...
/// Raw image buffer in HWC (height, width, channels) format with RGB pixel order.
#[derive(Debug, Clone)]
...
/// Preprocessing configuration matching `get_val_transforms(256)`.
#[derive(Debug, Clone)]
pub struct PreprocessConfig {
    /// Final target size after center crop.
    pub target_size: u32,
    /// Resize size before center crop (target_size * 1.1).
    pub resize_size: u32,
...
/// Preprocess an image for disease detection inference.
///
/// Pipeline: resize -> center crop -> normalize -> CHW float tensor.
pub fn preprocess_image(
    image: &ImageBuffer,
    config: &PreprocessConfig,
) -> Result<Array3<f32>, PreprocessError> {
    // 1. Bilinear resize to resize_size x resize_size
...
pub fn preprocess_batch(
    ...
    images
        .par_iter()
        .map(|img| preprocess_image(img, config))
        .collect()
}
...
mod tests {
    ...
    fn test_preprocess_output_shape() {
        ...
        assert_eq!(result.shape(), &[3, 256, 256]);
    }

    #[test]
    fn test_preprocess_batch() {
        let images = vec![make_test_image(200, 200), make_test_image(400, 300)];
        let config = PreprocessConfig::default();
        let results = preprocess_batch(&images, &config).unwrap();
        assert_eq!(results.len(), 2);
        assert_eq!(results[0].shape(), &[3, 256, 256]);
    }
}
