//! Image preprocessing for plant classification inference.
//!
//! Mirrors the Python transforms from `plant_classification/transforms.py`:
//! 1. Resize to 246x246 (224 * 1.1)
...
/// Default input image size for plant classification (ResNet50).
...
/// Raw RGB image buffer in HWC format.
#[derive(Debug, Clone)]
...
/// Preprocessing configuration matching ResNet50 validation transforms.
#[derive(Debug, Clone)]
...
/// Preprocess an image for plant classification inference.
///
...
mod tests {
    ...
    fn test_preprocess_batch() {
        ...
        assert_eq!(results[0].shape(), &[3, 224, 224]);
    }
