// Package main wires device-service's layers together and starts the server.
package main
...
import (
	...
	"p9e.in/samavaya/agriculture/device-service/api/v1/devicev1connect"
	grpcadapter "p9e.in/samavaya/agriculture/device-service/internal/adapters/inbound/grpc"
	kafkaadapter "p9e.in/samavaya/agriculture/device-service/internal/adapters/outbound/kafka"
	postgresadapter "p9e.in/samavaya/agriculture/device-service/internal/adapters/outbound/postgres"
	"p9e.in/samavaya/agriculture/device-service/internal/application"
)
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/device_service?sslmode=disable")
	...
	repo := postgresadapter.NewDeviceRepository(pool, logger)
	pub := kafkaadapter.NewEventPublisher(kafkaProducer, logger)
	svc := application.NewDeviceService(repo, pub, logger)
	handler := grpcadapter.NewDeviceHandler(svc, logger)
	...
	path, deviceHandler := devicev1connect.NewDeviceServiceHandler(handler,
