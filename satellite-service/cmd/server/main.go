// Package main wires all satellite-service layers together and starts the HTTP server.
// Pre-requisites: run `buf generate` in satellite-service/ to produce api/v1/*.pb.go
package main
...
import (
	...
	// Generated by buf generate — run `buf generate` in satellite-service/ first.
	satellitev1connect "p9e.in/samavaya/agriculture/satellite-service/api/v1/satellitev1connect"

	eventsadapter "p9e.in/samavaya/agriculture/satellite-service/internal/adapters/inbound/events"
	grpcadapter "p9e.in/samavaya/agriculture/satellite-service/internal/adapters/inbound/grpc"
	clientsadapter "p9e.in/samavaya/agriculture/satellite-service/internal/adapters/outbound/clients"
	kafkaadapter "p9e.in/samavaya/agriculture/satellite-service/internal/adapters/outbound/kafka"
	postgresadapter "p9e.in/samavaya/agriculture/satellite-service/internal/adapters/outbound/postgres"
	"p9e.in/samavaya/agriculture/satellite-service/internal/application"
)
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/satellite_service?sslmode=disable")
	kafkaBroker := os.Getenv("KAFKA_BROKER")
	fieldServiceURL := envOr("FIELD_SERVICE_URL", "http://localhost:8081")
	farmServiceURL := envOr("FARM_SERVICE_URL", "http://localhost:8082")
	...
	// Inbound adapters
	handler := grpcadapter.NewSatelliteHandler(svc, logger)
	...
	mux.Handle(path, satelliteHandler)
	...
	if kafkaBroker != "" {
		eventConsumer := eventsadapter.NewSatelliteConsumer(svc, logger)
		kc := kafkaconsumer.NewKafkaConsumer(&kafkaconfig.KafkaConfig{
			Broker:       kafkaBroker,
			Group:        "satellite-service",
			KafkaVersion: "3.5.0",
			Assignor:     "sticky",
		}, logger)
	...
	go func() {
		p9log.NewHelper(logger).Infow("msg", "satellite-service starting", "port", port)
	...
	<-quit
	p9log.NewHelper(logger).Infow("msg", "shutting down satellite-service")
