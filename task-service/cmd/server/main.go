// Package main wires all task-service layers together and starts the HTTP server.
package main
...
import (
	...
	"p9e.in/samavaya/agriculture/task-service/internal/handlers"
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/task_service?sslmode=disable")
	...
	// ── Application layers ──────────────────────────────────────────────────
	repo := repositories.NewTaskRepository(pool, logger)
	svc := services.NewTaskService(repo, outboxPub, pool, logger)
	handler := handlers.NewTaskHandler(d, svc)
	...
	path, svcHandler := v1connect.NewTaskServiceHandler(handler,
	...
	go func() {
		p9log.NewHelper(logger).Infow("msg", "task-service starting", "port", port)
	...
	<-quit
	p9log.NewHelper(logger).Infow("msg", "shutting down task-service")
