<script lang="ts">
  ...
  // Yield records are returned by GetYieldHistory; there is no ListYieldRecords.
...
<EntityListPage
  ...
  createHref="/crop-intelligence/historical-yields/new"
  {columns}
  rows={rows as any}
  {loading}
  {error}
  {totalCount}
  onRowClick={(id) => goto(`/crop-intelligence/historical-yields/${id}`)}
  {fetchData}
/>
