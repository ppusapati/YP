...
import (
	...
	eventsadapter "p9e.in/samavaya/agriculture/sensor-service/internal/adapters/inbound/events"
...
func main() {
	...
	// Application service
	svc := application.NewSensorService(repo, outboxPub, fieldClient, farmClient, pool, logger)
	...
	if kafkaBroker != "" {
		eventConsumer := eventsadapter.NewSensorConsumer(svc, logger)
		kc := kafkaconsumer.NewKafkaConsumer(&kafkaconfig.KafkaConfig{
			Broker:       kafkaBroker,
			Group:        "sensor-service",
			KafkaVersion: "3.5.0",
			Assignor:     "sticky",
		}, logger)
