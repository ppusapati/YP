// Package main wires all crop-service layers together and starts the HTTP server.
// Pre-requisites: run `buf generate` in crop-service/ to produce api/v1/*.pb.go
package main
...
import (
	...
	// Generated by buf generate — run `buf generate` in crop-service/ first.
	cropv1connect "p9e.in/samavaya/agriculture/crop-service/api/v1/cropv1connect"

	eventsadapter "p9e.in/samavaya/agriculture/crop-service/internal/adapters/inbound/events"
	"p9e.in/samavaya/agriculture/crop-service/internal/ai"
	grpcadapter "p9e.in/samavaya/agriculture/crop-service/internal/adapters/inbound/grpc"
	kafkaadapter "p9e.in/samavaya/agriculture/crop-service/internal/adapters/outbound/kafka"
	postgresadapter "p9e.in/samavaya/agriculture/crop-service/internal/adapters/outbound/postgres"
	"p9e.in/samavaya/agriculture/crop-service/internal/application"
)
...
func main() {
	...
	// Build the Kafka producer directly via sarama.
	...
	svc := application.NewCropService(repo, outboxPub, pool, logger, aiClient)
	...
	const serviceName = "crop-service"
	...
	if kafkaBroker != "" {
		eventConsumer := eventsadapter.NewCropConsumer(svc, logger)
	...
	go func() {
		p9log.NewHelper(logger).Infow("msg", "crop-service starting", "port", port)
	...
	<-quit
	p9log.NewHelper(logger).Infow("msg", "shutting down crop-service")
