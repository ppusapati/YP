...
import (
	...
	kafkaproducer "p9e.in/samavaya/packages/events/producer"
...
func main() {
	...
	// Build Kafka producer via the packages/events layer.
	var kp *kafkaproducer.KafkaProducer
	if kafkaBroker != "" {
		kp = kafkaproducer.NewKafkaProducer(&kafkaconfig.KafkaConfig{
			Broker:        kafkaBroker,
			RetryMax:      3,
			RetryInterval: 1,
		}, logger)
		if kp != nil {
			...
			log.Printf("WARNING: failed to create Kafka producer")
		}
	}

	// Build ServiceDeps for the rich service/repository layer.
	d := deps.ServiceDeps{
		Pool:          pool,
		Log:           logger,
		KafkaProducer: kp,
	...
	// Rich repository and service
	repo := repositories.NewCropRepository(d)
	svc := services.NewCropService(d, repo)
