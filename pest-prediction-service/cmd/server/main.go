...
func main() {
	...
	// Optional: without it, pest risk falls back to the weather supplied with
	// the request rather than the field's own observations.
	var weatherClient outbound.WeatherClient
	if weatherServiceURL != "" {
		weatherClient = clientsadapter.NewWeatherClient(weatherServiceURL, connectclient.NewHTTPClient(connectclient.DefaultConfig(weatherServiceURL)), connect.WithInterceptors(connectclient.ContextPropagator()))
	}
	...
	svc := application.NewPestService(repo, outboxPub,
		...
		sensorClient,
		farmClient,
		pool, logger, aiClient, weatherClient)
