// Package main wires all satellite-tile-service layers together and starts the HTTP server.
package main
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/satellite_tile_service?sslmode=disable")
	...
	const serviceName = "satellite-tile-service"
	path, svcHandler := v1connect.NewSatelliteTileServiceHandler(handler,
	...
	go func() {
		p9log.NewHelper(logger).Infow("msg", "satellite-tile-service starting", "port", port)
	...
	<-quit
	p9log.NewHelper(logger).Infow("msg", "shutting down satellite-tile-service")
