// Package main wires all prescription-service layers together and starts the HTTP server.
package main
...
import (
	...
	"p9e.in/samavaya/agriculture/prescription-service/internal/handlers"
...
func main() {
	...
	// Application service
	svc := services.NewPrescriptionService(d)

	// Handler
	handler := handlers.NewPrescriptionHandler(d, svc)
	...
	path, svcHandler := v1connect.NewPrescriptionServiceHandler(handler,
	...
	go func() {
		p9log.NewHelper(logger).Infow("msg", "prescription-service starting", "port", port)
	...
	<-quit
	p9log.NewHelper(logger).Infow("msg", "shutting down prescription-service")
