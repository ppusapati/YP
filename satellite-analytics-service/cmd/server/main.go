// Package main wires all satellite-analytics-service layers together and starts the HTTP server.
package main
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/satellite_analytics_service?sslmode=disable")
	...
	const serviceName = "satellite-analytics-service"
	path, svcHandler := v1connect.NewSatelliteAnalyticsServiceHandler(handler,
	...
	go func() {
		p9log.NewHelper(logger).Infow("msg", "satellite-analytics-service starting", "port", port)
	...
	<-quit
	p9log.NewHelper(logger).Infow("msg", "shutting down satellite-analytics-service")
