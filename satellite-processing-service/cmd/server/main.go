// Package main wires all satellite-processing-service layers together and starts the HTTP server.
package main
...
import (
	...
	"p9e.in/samavaya/agriculture/satellite-processing-service/internal/handlers"
...
func main() {
	...
	dsn := envOr("DATABASE_URL", "postgres://localhost:5432/satellite_processing_service?sslmode=disable")
	...
	repo := repositories.NewProcessingRepository(d)
	svc := services.NewProcessingService(d, repo)
	handler := handlers.NewProcessingHandler(d, svc)
	...
	const serviceName = "satellite-processing-service"
	path, svcHandler := v1connect.NewSatelliteProcessingServiceHandler(handler,
	...
	go func() {
		p9log.NewHelper(logger).Infow("msg", "satellite-processing-service starting", "port", port)
	...
	<-quit
	p9log.NewHelper(logger).Infow("msg", "shutting down satellite-processing-service")
