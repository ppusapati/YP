...
import (
	...
	eventsadapter "p9e.in/samavaya/agriculture/plant-diagnosis-service/internal/adapters/inbound/events"
...
func main() {
	...
	// Application service
	svc := application.NewDiagnosisService(repo, outboxPub,
		fieldClient,
		farmClient,
	...
	if kafkaBroker != "" {
		eventConsumer := eventsadapter.NewDiagnosisConsumer(svc, logger)
		kc := kafkaconsumer.NewKafkaConsumer(&kafkaconfig.KafkaConfig{
			Broker:       kafkaBroker,
			Group:        "plant-diagnosis-service",
			KafkaVersion: "3.5.0",
			Assignor:     "sticky",
		}, logger)
